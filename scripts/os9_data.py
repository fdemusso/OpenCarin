"""Mappa l'area dati statica di un modulo OS-9/68K e risolve i riferimenti a6.

M$IData (+0x40) punta a una lista di blocchi `{u32 dest_offset, u32 size, bytes}`
terminata da dest_offset/size = 0: e' l'immagine dei dati inizializzati, copiata
nell'area di storage statico del processo all'avvio.

Nel codice generato dal compilatore C Microware, A6 punta all'area dati + 0x8000,
quindi una variabile a offset D si legge come `(D - 0x8000)(a6)`.

Comandi:
    idata <modulo>              elenca i blocchi di dati inizializzati
    strings <modulo>            stringhe con offset dati e displacement a6
    xref <modulo> <sottostringa>  trova nel disassemblato chi referenzia la stringa
"""
