"""Grafo delle chiamate di un modulo OS-9000/MIPS.

Le chiamate sono PIC:  ``lui $at,0 ; addiu $at,$at,IMM ; addu $at,$at,$fp ; jalr $at``
quindi l'offset di modulo della callee e' ``0x7ff0 + IMM`` (IMM con segno).
Le funzioni iniziano con ``sw $ra, ($sp)`` oppure ``addiu $sp,$sp,-N``.

Comandi:
    funcs <modulo>                elenca le funzioni e quante chiamate fanno
    callers <modulo> <off>        chi chiama quell'offset
    calls <modulo> <off>          cosa chiama la funzione a quell'offset
"""
