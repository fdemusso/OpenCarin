"""Spatial lookup for CARiN 0x0E parcels.

Given a geographic point (x, y) in CARiN coordinates, returns the virtual sector of
the 0x0E block whose road-network anchor points best cover that location.

Algorithm
---------
On first call the function scans all 74,247 0x0E blocks, extracts the bounding box
of their S2 anchor coordinates (x_anc / y_anc), and saves a compact index to
``dataset/parcel_index.npz``.  Subsequent calls load from the cache (<1 s).

Index schema (int64 columns):
  [sector, x_min, y_min, x_max, y_max, cx, cy]
  cx / cy = centroid of all non-zero anchor points in the block.

Lookup
------
1. Find all blocks whose bbox contains (x, y).
2. Among those, return the sector whose centroid is nearest to (x, y).
3. If no bbox matches, fall back to nearest centroid overall.

Coordinate system
-----------------
K = 2_000_000_000 / 360 ≈ 5_555_555.56 units/degree; origin 30° W, equator.
  X = (lon + 30) * K        Y = lat * K
"""
