"""Disassemblatore m68k module-aware per i firmware OS-9 di CARIN.

Il codice di un modulo OS-9/68K eseguibile comincia a M$Exec (offset +0x30
nell'header) e finisce a M$IData (+0x40), dove inizia l'area dati inizializzata.
Le funzioni compilate in C hanno il prologo `link a5,#-N` e finiscono con
`unlk a5 ; rts`.

Comandi:
    funcs <modulo>            elenca le funzioni con statistiche
    dis   <modulo> <off> [n]  disassembla n istruzioni da un offset di modulo
    scan  <modulo>            classifica le funzioni per densita' di shift/bit
"""
